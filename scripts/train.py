...
def get_hparams(method, args):
    ...
    hparam_names = set(get_pipeline_hparams(args.method).keys())
    ...
    if args.param is not None:
        for param_name, param_val in args.param:
            if param_name not in hparam_names:
                ...
                exit(-1)
            if param_val.lower() in ('yes', 'true'):
                param_val = True
            elif param_val.lower() in ('no', 'false'):
                param_val = False
            else:
                try:
...
if __name__ == '__main__':
    ...
    group.add_argument('dataset', type=str, choices=available_datasets(),
    ...
    group.add_argument('--train-split', type=str, default='trainval',
    ...
    group.add_argument('--weight-decay', type=float, default=0.001,
                       help='Weight decay.')
    group.add_argument('--param', type=str, action='append', nargs=2, metavar=('NAME', 'VALUE'),
