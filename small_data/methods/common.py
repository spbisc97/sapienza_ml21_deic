    ...
    **hparams
    ...
    def create_model(self, arch: str, num_classes: int, input_channels: int = 3) -> nn.Module:
        ...
        arch : str
            ...
            `small_data.classifiers.available_classifiers`.
        num_classes : int
        ...
        return build_classifier(arch=arch, num_classes=num_classes, input_channels=input_channels)
    ...
    def get_optimizer(self, model: nn.Module, max_epochs: int, max_iter: int) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=self.hparams['lr'], weight_decay=self.hparams['weight_decay'])
